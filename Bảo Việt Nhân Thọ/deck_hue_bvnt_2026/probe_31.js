...
slides.slice(0,31).forEach((s,idx)=>{
const n=idx+1;
if(s.type==="hero"){
...
if(s.type==="impact"){
  content(n,"Expected Impact","Một nền tảng tốt phải tạo tác động đồng thời lên awareness, trust, perception và business momentum.","Tác động lớn nhất không chỉ ở reach, mà ở cách thương hiệu được nhìn nhận sau sự kiện.",sl=>{
    ...
    sl.addChart(pptx.ChartType.bar,[{name:"Tác động dự kiến",labels:["Awareness","Trust","Cultural relevance","Social buzz","Lead interest"],values:[88,83,90,86,72]}],{x:0.95,y:3.56,w:5.85,h:2.35,catAxisLabelFontFace:"Barlow",valAxisLabelFontFace:"Barlow",chartColors:[C.b,C.g,C.bd,"3E8DD1",C.gd],showLegend:false,valGridLine:{color:"E6ECF2",pt:1},valAxisMinVal:0,valAxisMaxVal:100,showValue:true,dataLabelColor:C.t,dataLabelPosition:"outEnd"});
...
(async()=>{const out=path.join(__dirname,"output","probe_31.pptx");await pptx.writeFile({fileName:out});console.log(`Wrote ${out}`);})();
